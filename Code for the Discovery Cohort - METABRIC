...
# Note: n.o. NAs are 529 --> diff between 2509 (clinical) and 1980 (1980) is 529 --> patients who are not subtyped are not included in the mRNA data
...
ggplot(df_melted_selected, aes(x = PAM50, y = value, fill = PAM50)) +   # Violin plot of immune cell fractions grouped by PAM50 subtype
  ...
  stat_compare_means(comparisons = subtype_comparisons, label = "p.signif") +  # Shows significant and non-significant values from pairwise comparison
...
# Kaplan-Meier curve for the 6 active celltypes according to the heatmap (Note: no subtype classification here)
...
# Kaplan-Meier curve for each subtype for the 6 active celltypes according to the heatmap
...
### SURVIVAL ANALYSIS: Multivarible -------------------------------------------------------------------------------------------------------------------------------------------------
